...
/**
 * Verifica se o usuário é um administrador SUPER (role = 'admin' apenas)
 * Diferente de isAdmin() que também inclui 'administrativo'
 */
 ...
 */
/**
 * Compara dados antigos com novos e retorna se há diferenças
 * Ignora campos como updated_at e criados_at que mudam automaticamente
 * @param array $oldData Dados antigos
 * @param array $newData Dados novos
 * @param array $fieldsToIgnore Campos a ignorar na comparação (padrão: updated_at, created_at)
 * @return bool true se há diferenças, false se tudo igual
 */
function hasDataChanged($oldData, $newData, $fieldsToIgnore = ['updated_at', 'created_at']) {
    if (!is_array($oldData) || !is_array($newData)) {
        return true;
    }
    
    // Pega a lista de campos em ambos os arrays
    $allKeys = array_unique(array_merge(array_keys($oldData), array_keys($newData)));
    
    foreach ($allKeys as $key) {
        // Ignora campos que não deveríamos comparar
        if (in_array(strtolower($key), array_map('strtolower', $fieldsToIgnore))) {
            continue;
        }
        ...
        // Comparação rigorosa: "0" != 0, false != null, etc
