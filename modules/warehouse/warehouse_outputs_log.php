<?php
// ========================================
// PÁGINA DE LOG DE SAÍDA DO ARMAZÉM
// ========================================
...
$page_title = 'Histórico de Saídas do Armazém';
...
try {
    ...
    if (!empty($search)) {
        $where_conditions[] = "(wo.warehouse_name LIKE ? OR wo.reason LIKE ? OR wo.details LIKE ? OR u.username LIKE ?)";
        $params = array_fill(0, 4, "%{$search}%");
    }
    if (!empty($date_from)) {
        $where_conditions[] = "DATE(wo.output_date) >= ?";
        $params[] = $date_from;
    }
    if (!empty($date_to)) {
        $where_conditions[] = "DATE(wo.output_date) <= ?";
    ...
    $count_query = "SELECT COUNT(*) as total FROM warehouse_outputs wo LEFT JOIN users u ON wo.user_id = u.id {$where_clause}";
    ...
    error_log("Erro ao carregar dados de saída do armazém: " . $e->getMessage());
    $outputs = [];
    $total_records = 0;
    $total_pages = 0;
}
...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h2 text-primary-custom">
        ...
        Histórico de Saídas do Armazém
    </h1>
</div>
    ...
    <div class="card-body p-0">
        <?php if (empty($outputs)): ?>
            <div class="text-center text-muted py-5">
                ...
                <p>Nenhuma saída do armazém registrada.</p>
            </div>
        <?php else: ?>
                    ...
                    <tbody>
                        <?php foreach ($outputs as $output): ?>
                            <tr>
                                ...
                                <td>
                                    <strong><?php echo htmlspecialchars($output['warehouse_name']); ?></strong>
                                    <br><small class="text-muted"><?php echo htmlspecialchars($output['warehouse_category'] ?? 'N/A'); ?></small>
                                </td>
