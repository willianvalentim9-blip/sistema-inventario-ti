...
$page_title = 'Editar Garantia do Armazém';
$warehouse_id = intval($_GET['id'] ?? $_POST['id'] ?? 0);

if ($warehouse_id <= 0) {
    $error_msg = 'ID de item inválido.';
    ...
    header('Location: warranties.php?tab=warehouse');
    exit;
}

try {
    $pdo = getConnection();
    $stmt = $pdo->prepare("SELECT id, name, description, has_warranty, warranty_start_date, warranty_end_date, warranty_provider, warranty_notes, warranty_period_value, warranty_period_unit, warranty_ticket_number, warranty_label, warranty_client_name, invoice_number, warranty_supplier_id FROM warehouse WHERE id = ? AND (is_deleted = FALSE OR is_deleted IS NULL)");
    $stmt->execute([$warehouse_id]);
    $warehouse = $stmt->fetch();

    if (!$warehouse) {
        $error_msg = 'Item do armazém não encontrado.';
    ...
    $had_warranty_before = ($warehouse['has_warranty'] == 1);
} catch (PDOException $e) {
    error_log("Erro ao buscar garantia do armazém: " . $e->getMessage());
    ob_end_clean();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($warranty_start_date) || empty($warranty_period_value) || empty($warranty_period_unit) || empty($warranty_end_date)) {
        ...
        $_SESSION['flash_message'] = $error_msg;
        $_SESSION['flash_type'] = 'danger';
        header('Location: edit_warranty_warehouse.php?id=' . $warehouse_id . ($is_modal ? '&modal=true' : ''));
        exit;
    }

    // Validação (Start Date > End Date)
    ...
    // Validação da consistência do período
    ...
    try {
        ...
        $new_data_stmt = $pdo->prepare("SELECT id, name, description, has_warranty, warranty_start_date, warranty_end_date, warranty_provider, warranty_notes, warranty_period_value, warranty_period_unit, warranty_ticket_number, warranty_label, warranty_client_name, invoice_number, warranty_supplier_id FROM warehouse WHERE id = ?");
        ...
        if (!$had_warranty_before) {
            ...
            $return_to = $_POST['return_to'] ?? 'warehouse.php';
        ...
        $pdo->rollBack();
        error_log("Erro ao atualizar garantia do armazém: " . $e->getMessage());
    ...
    <div class="row justify-content-center">
        <div class="col-lg-8 col-md-10">
            <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                ...
                <a href="warranties.php?tab=warehouse" class="btn btn-outline-secondary btn-sm">
...
<?php endif; ?>
            ...
            <div class="card card-custom">
                <div class="card-header card-header-custom">
                    <h5 class="card-title mb-0">Item do Armazém: <?php echo htmlspecialchars($warehouse['name']); ?></h5>
                </div>
                <div class="card-body">
                    <form method="POST" action="edit_warranty_warehouse.php?id=<?php echo $warehouse['id']; ?><?php echo $is_modal ? '&modal=true' : ''; ?>" id="editWarrantyWarehouseForm">

                        <!-- Campo hidden para retornar à página de origem -->
                        <input type="hidden" name="return_to" value="<?php echo htmlspecialchars($_GET['return_to'] ?? $_SERVER['HTTP_REFERER'] ?? 'warehouse.php'); ?>">
                        ...
                        <div id="duplicate-warranty-alert" class="alert alert-warning alert-dismissible fade show d-none" role="alert">
                            ...
                            <strong>⚠️ Atenção!</strong> Este item já possui uma garantia registrada com os mesmos dados.
                            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                        </div>
                        
                        <div id="warehouse-modified-alert" class="alert alert-info alert-dismissible fade show d-none" role="alert">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" class="form-control form-control-custom" id="warranty_client_name" name="warranty_client_name" placeholder="Nome completo do cliente" value="<?php echo htmlspecialchars($warehouse['warranty_client_name'] ?? ''); ?>" required>
                                <small class="text-muted">Cliente proprietário do item com garantia</small>
                            </div>
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" class="form-control form-control-custom" id="warranty_ticket_number" name="warranty_ticket_number" placeholder="Ex: TKT-2025-001234" value="<?php echo htmlspecialchars($warehouse['warranty_ticket_number'] ?? ''); ?>">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" class="form-control form-control-custom" id="warranty_label" name="warranty_label" placeholder="Ex: GRT-SAMSUNG-2025-001" value="<?php echo htmlspecialchars($warehouse['warranty_label'] ?? ''); ?>">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" class="form-control form-control-custom" id="warranty_provider" name="warranty_provider" placeholder="Ex: Samsung, LG, Autorizada..." value="<?php echo htmlspecialchars($warehouse['warranty_provider'] ?? ''); ?>">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" class="form-control form-control-custom" id="invoice_number" name="invoice_number" placeholder="Ex: NF 123456789" value="<?php echo htmlspecialchars($warehouse['invoice_number'] ?? ''); ?>">
                        ...
                        <div class="row">
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="date" class="form-control form-control-custom" id="warranty_start_date" name="warranty_start_date" value="<?php echo htmlspecialchars($warehouse['warranty_start_date'] ?? ''); ?>" required>
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <div class="input-group">
                                    <input type="number" class="form-control form-control-custom" id="warranty_period_value" name="warranty_period_value" min="1" placeholder="Ex: 12" value="<?php echo htmlspecialchars($warehouse['warranty_period_value'] ?? ''); ?>" required>
                                    <select class="form-select form-control-custom" id="warranty_period_unit" name="warranty_period_unit">
                                        <option value="days" <?php echo ($warehouse['warranty_period_unit'] ?? '') === 'days' ? 'selected' : ''; ?>>Dias</option>
                                        <option value="months" <?php echo ($warehouse['warranty_period_unit'] ?? 'months') === 'months' ? 'selected' : ''; ?>>Meses</option>
                                        <option value="years" <?php echo ($warehouse['warranty_period_unit'] ?? '') === 'years' ? 'selected' : ''; ?>>Anos</option>
                                    </select>
                                </div>
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="date" class="form-control form-control-custom" id="warranty_end_date" name="warranty_end_date" value="<?php echo htmlspecialchars($warehouse['warranty_end_date'] ?? ''); ?>" required>
...
<script>
const originalData = {
    warranty_provider: '<?php echo htmlspecialchars($warehouse['warranty_provider'] ?? ''); ?>',
    warranty_start_date: '<?php echo htmlspecialchars($warehouse['warranty_start_date'] ?? ''); ?>',
    warranty_period_value: '<?php echo htmlspecialchars($warehouse['warranty_period_value'] ?? ''); ?>',
    warranty_period_unit: '<?php echo htmlspecialchars($warehouse['warranty_period_unit'] ?? ''); ?>',
    warranty_end_date: '<?php echo htmlspecialchars($warehouse['warranty_end_date'] ?? ''); ?>',
    warranty_notes: `<?php echo htmlspecialchars($warehouse['warranty_notes'] ?? ''); ?>`,
    warranty_ticket_number: '<?php echo htmlspecialchars($warehouse['warranty_ticket_number'] ?? ''); ?>',
    warranty_label: '<?php echo htmlspecialchars($warehouse['warranty_label'] ?? ''); ?>',
    warranty_client_name: '<?php echo htmlspecialchars($warehouse['warranty_client_name'] ?? ''); ?>',
    warranty_supplier_id: '<?php echo htmlspecialchars($warehouse['warranty_supplier_id'] ?? ''); ?>',
    invoice_number: '<?php echo htmlspecialchars($warehouse['invoice_number'] ?? ''); ?>'
};
...
function initializeWarrantyListeners() {
    ...
    const modifiedAlert = document.getElementById('warehouse-modified-alert');
    ...
    function calculateEndDate() {
        ...
        console.log('Valores:', { startDate, periodValue, periodUnit });

        if (!startDate || periodValue <= 0 || !periodUnit) {
            console.log('❌ Dados insuficientes para calcular:', { startDate, periodValue, periodUnit });
            return;
        }
        ...
        try {
            ...
            console.log('📅 Data calculada:', calculatedDate);
            endDateInput.value = calculatedDate;
            console.log('✅ Data final calculada e definida:', calculatedDate);
    ...
    periodValueInput.addEventListener('keyup', calculateEndDate);
    ...
    // ========================================
    // INTERCEPTA CLIQUE EM TEMPLATES
    // ========================================
    document.addEventListener('click', function(e) {
        if (e.target.closest('.template-btn')) {
            // Quando um template é clicado, não salvamos o ID
            // Os dados são aplicados diretamente aos campos
            console.log('📋 Template aplicado - aguardando salvamento');
        }
    });
    ...
    // ========================================
    const form = document.getElementById('editWarrantyWarehouseForm');
...
if (document.getElementById('editWarrantyWarehouseForm')) {
