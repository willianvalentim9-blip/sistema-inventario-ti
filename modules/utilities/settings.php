    ...
    <div class="col-lg-4">
        <div class="card card-custom mb-4">
            ...
            <div class="card-body text-center">
                ...
                $logo_exists = !empty($logo_path) && file_exists($full_logo_path);
                ?>
                <div id="logo-display-area" class="mb-3">
                    <?php if ($logo_exists): ?>
                        <img src="/sistema5/<?php echo htmlspecialchars($logo_path) . '?v=' . time(); ?>" alt="Logo da Empresa" id="logo-preview" class="img-thumbnail" style="max-height: 80px;">
                    <?php else: ?>
        ...
        <div class="card card-custom mb-4">
            ...
            <div class="card-body">
                ...
                <div class="d-grid gap-2">
                    <a href="<?php echo url('modules/logs/admin_logs.php'); ?>" class="btn btn-outline-secondary"><i class="fas fa-shield-alt me-2"></i>Logs de Administrador</a>
                    <a href="<?php echo url('dev/diagnostico/'); ?>" class="btn btn-outline-info" target="_blank"><i class="fas fa-stethoscope me-2"></i>Central de Diagnósticos</a>
                </div>
            </div>
        </div>
    </div>
...
<script>
// Função para exibir detalhes da alteração no modal
...
// Proteção global para evitar inicialização duplicada
if (!window.logoSettingsInitialized) {
    window.logoSettingsInitialized = true;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (logoInput && !logoInput.dataset.listenerAdded) {
            ...
            logoInput.addEventListener('change', function handleLogoChange(e) {
                ...
                if (this.files && this.files[0]) {
                    ...
                    fetch('<?php echo url('modules/utilities/update_logo.php'); ?>', { method: 'POST', body: formData })
                        .then(response => response.json())
                        .then(data => {
                            logoInput.dataset.uploading = 'false';
                            logoInput.value = '';
        ...
        if (removeBtn && !removeBtn.dataset.listenerAdded) {
            ...
            removeBtn.addEventListener('click', function handleLogoRemove() {
                showConfirmation({
                    title: 'Remover Logo',
