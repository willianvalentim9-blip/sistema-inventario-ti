...
try {
    ...
    // BUSCAR DADOS
    // ========================================
    $data = getMovements($pdo, 'saida', $search, $filter_date_from, $filter_date_to, $page, $per_page);
    
    // Contar totais por tipo
    $count_query = "SELECT COUNT(*) as total FROM product_movements WHERE movement_type = 'saida'";
    $count_stmt = $pdo->prepare($count_query);
    $count_stmt->execute();
    $total_count = $count_stmt->fetch()['total'];
    
} catch (Exception $e) {
    error_log("Erro em movementations_saida.php: " . $e->getMessage());
    ...
    <div class="tab-pane fade <?php echo $tab === 'produtos' ? 'show active' : ''; ?>">
        <?php if (empty($data['movements'])): ?>
            <div class="alert alert-info">
                <i class="fas fa-info-circle me-2"></i>Nenhuma saída registrada
            </div>
        <?php else: ?>
